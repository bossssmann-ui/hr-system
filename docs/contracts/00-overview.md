...
- **Quiet Hours** (Phase 1E): automated outbound messages are deferred outside the active send window. This company spans Vladivostok (UTC+10) → Moscow (UTC+3), so the active window is 09:00 Vladivostok → 18:00 Moscow = **23:00 UTC → 15:00 UTC**. The window is configurable via `QUIET_HOURS_QUIET_START_UTC` / `QUIET_HOURS_QUIET_END_UTC` env vars (defaults: quiet 15–23 UTC). The central helper lives in `backend/src/features/messaging/quiet-hours.ts` and must be imported by any future automated-outbound path (scheduled jobs, Notifier transports, etc.) rather than reimplementing the logic.
